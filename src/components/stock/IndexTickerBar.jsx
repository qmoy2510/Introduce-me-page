...
function TickerItem({ label, quote, loading }) {
  ...
  const price = isNaN(quote.price) ? '-' : quote.price.toLocaleString(undefined, { maximumFractionDigits: 2 })
  const pct = isNaN(quote.changePct) ? '-' : Math.abs(quote.changePct).toFixed(2) + '%'
  return (
    <div className="flex items-center gap-3 px-4 py-2 bg-surface rounded-lg min-w-[160px] shrink-0">
      ...
      <span className="text-text text-sm font-semibold">{price}</span>
